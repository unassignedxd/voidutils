package com.github.unassignedxd.voidutils.main.init;

import com.github.unassignedxd.voidutils.main.blocks.BlockCatalyticInfuser;
import com.github.unassignedxd.voidutils.main.blocks.BlockCrystal;
import com.github.unassignedxd.voidutils.main.blocks.BlockCrystallizer;
import com.github.unassignedxd.voidutils.main.blocks.BlockInfuser;
import net.minecraft.block.Block;

public class ModBlocks {

    /**
     * Functional Blocks - [VOID INFUSION]
     */
    public static final Block INFUSER = new BlockInfuser("infuser");
    public static final Block CATALYTIC_INFUSER = new BlockCatalyticInfuser("catalytic_infuser");

    public static final Block CRYSTALLIZER = new BlockCrystallizer("crystallizer");
    public static final Block CRYSTAL = new BlockCrystal("crystal");

}
